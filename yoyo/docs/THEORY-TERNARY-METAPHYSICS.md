...
复现结论**读存档即可**，或打开 **[`结论.md`](../../结论.md)** 一览；投票逻辑用 `yoyo/research/walk_forward.ty` 编译验证。
...
你描述的 **一涨、二跌、三无问**，对应本系统 trit：**2 买 / 0 卖 / 1 持**——这是**单个人**的动作空间。

盘面上「惊人一致」的涨跌停、放量长阳阴，却来自**聚合与耦合**：

| 力量 | 作用 | 本仓库对应 |
|------|------|-----------|
| 共同信息冲击 | 政策、业绩、地缘 → 同向更新信念 | `perturbation.ty` 第 5 票 η |
| 价格正反馈 | 涨→追涨、跌→恐慌；微小差放大 | 蝴蝶效应演示 `butterfly-demo` |
| 制度与流动性 | 涨跌停、强平、指数跟踪 | 未建模（诚实边界） |
| 技术共振 | 众人盯同一套 K 线 | 四票 `indicators.ty` |

**三进制解释的是「如何把多路信号合成一个动作」（Σ trit 投票），不是「谁在发号令让亿万人同心」。**

实证上：archive v1 中 **corr(信号, 次日收益) ≈ 0.003** → 那种集体一致，多半是**事后**在价量上看见，**事前**用简单 trit 规则很难稳定预测。

> 个人三态，市场二象（涨跌可见）；「无问」的人不交易，却被成交量隐藏。  
> 详见仓库根目录 **[`结论.md`](../../结论.md)**（结论一览，推荐先读）。
...
| 文件 | 内容 |
|------|------|
| **`结论.md`**（仓库根目录） | **结论一览：实证数字 + 集体一致 + 三进制边界** |
