#!/usr/bin/awk -f
# 震巽股票 v4 回测（纯 awk）：七票 + 新闻 η + 历史逐笔（有则启用）
# 辅助数据：-v newsfile= -v tickfile= -v code=
...
END {
    ...
    printf("%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
        code, name, days,
        pct_buy, pct_hold, pct_sell,
        mean_wuwen, mean_calm, mean_consensus,
        eq_v4 - 1, eq_mom - 1, eq_mr - 1, eq_bh - 1,
        sh_v4, sh_mom, sh_mr,
