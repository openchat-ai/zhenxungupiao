#!/usr/bin/awk -f
# 震巽股票 v3 回测引擎（纯 awk，零 Python）
# 六票：indicators + perturbation + psychology + wuwen
...
function decide_v3(i,    v1, v2, v3, v4, v5, v6, s) {
    v1 = vote_ma(i); v2 = vote_trend(i); v3 = vote_rsi_simple(i)
    v4 = vote_macd_simple(i); v5 = vote_perturb(i); v6 = vote_psych(i)
    s = v1 + v2 + v3 + v4 + v5 + v6
    votes_sum[i] = s
    hold_votes[i] = (v1==1) + (v2==1) + (v3==1) + (v4==1) + (v5==1) + (v6==1)
    wuwen_pct[i] = int(hold_votes[i] * 100 / 6 + 0.5)
...
END {
  ...
  wuwen_sum = calm_sum = cons_sum = 0
    for (i = 20; i <= n; i++) signal_v2[i] = decide_v3(i)
    for (i = 20; i <= n; i++) {
